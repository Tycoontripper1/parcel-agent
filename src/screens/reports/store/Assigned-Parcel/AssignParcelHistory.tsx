...
const AssignParcelHistory = ({ navigation }: Props) => {
  ...
  const downloadReport = async () => {
    ...
    const csvRows = filteredTransactions
      .map(
        (tx) =>
          `${tx.date},${tx.title},${tx.sender},${tx.receiver},₦${tx.amount}`
      )
      .join("\n");
            ...
            <View style={styles.section}>
              ...
              {groupedTransactions[properDate]?.map((item, index) => (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate("AssignParcelDetails", { id: item.id })
                  }
                >
...
export default AssignParcelHistory;
