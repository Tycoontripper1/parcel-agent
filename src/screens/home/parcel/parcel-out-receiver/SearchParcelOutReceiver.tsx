...
import { getAllParcel, getSingleParcelData } from '../../../../../services/parcel';
...
const [searchResults, setSearchResults] = useState<singleParcelInterface[]>([]); // ⬅ Store multiple parcels
  ...
  <View style={{ paddingTop: 10 }}>
    ...
    {searchResults.map((parcel, index) => (
      <TouchableOpacity
        key={index}
        onPress={async () => {
          await AsyncStorage.setItem('singleParcelData', JSON.stringify(parcel));
          navigation.navigate('ParcelReceiverOutPreviewScreen');
        }}
        style={styles.shipmentRow}
      >
        <Image source={{ uri: parcel?.parcel?.thumbnails[0] }} style={styles.shipmentImage} />
                 ...
                 <View style={styles.shipmentDetails}>
                   <View style={{flexDirection: 'column', gap: 4}}>
                     ...
                     <Text size={RFValue(10)}>{parcel.sender.fullName ? parcel.sender.fullName: parcel.sender.phone }</Text>
                   ...
                   <View style={{flexDirection: 'column', gap: 4}}>
                     ...
                     <Text size={RFValue(10)}>{parcel?.receiver.fullName ? parcel?.receiver.fullName : parcel?.receiver.phone}</Text>
